...
class PipelineConfig:
    bars: int = 16
    beats_per_bar: int = 4
    # Basic Pitch's native note matrix begins at MIDI 21. We no longer pass these
    # bounds into Basic Pitch because v0.4.0 can zero the matrix when min<21.
    # Decode first, then filter explicitly in our own event domain.
    ...
    drum_device: str = "cpu"  # ADTOF-pytorch currently exposes cpu/cuda only.
    bass_onset_threshold: float = 0.48
    bass_frame_threshold: float = 0.28
    bass_min_note_ms: float = 85.0
    ambiguous_amplitude: float = 0.55
    # Fail loud only when zero MIDI conflicts with strong independent evidence.
