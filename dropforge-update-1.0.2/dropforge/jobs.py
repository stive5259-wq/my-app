...
class JobManager:
    ...
    def public(self, job: Job) -> dict[str, Any]:
        return {"id": job.id, "stage": job.stage, "progress": job.progress, "message": job.message, "error": job.error, "result": job.result}

    async def run(self, job: Job, *, url: str | None, upload_path: Path | None) -> None:
        work = WORK_ROOT / job.id
        work.mkdir(parents=True, exist_ok=True)
        def cb(stage: str, pct: float, msg: str): self.emit(job, stage, pct, msg)
        try:
            analysis, bundle = await asyncio.to_thread(run_pipeline, work, url=url, local_path=upload_path, progress=cb)
            job.result = {
