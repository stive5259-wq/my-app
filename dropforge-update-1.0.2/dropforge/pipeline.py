...
def run_pipeline(work_dir: Path, *, url: str | None = None, local_path: Path | None = None, progress: Progress=lambda *_: None) -> tuple[Analysis, OutputBundle]:
    ...
    cache_names = [drums_clip.name, bass_clip.name, other_clip.name, key_source.name]
    if cache.restore_files("demix_section", demix_key, cache_names, work_dir):
        cache_hits["demix_section"] = True
        progress("demix", 0.45, "Loaded separated 16-bar stems from cache")
        ...
        cache.store_files("demix_section", demix_key, [drums_clip, bass_clip, other_clip, key_source])

    key = estimate_key(key_source, 0.0, duration)
    bass_audio_stats = signal_stats(bass_clip)
    diagnostics["bass_audio"] = bass_audio_stats
    ...
    write_midi(bass_raw_midi, bass_raw, section.bpm, False, "DropForge Bass RAW", bars=cfg.bars, beats_per_bar=cfg.beats_per_bar)
    ...
    write_midi(drums_midi, drums, section.bpm, True, "DropForge Drums", bars=cfg.bars, beats_per_bar=cfg.beats_per_bar)
    write_midi(bass_midi, bass, section.bpm, False, "DropForge Bass", bars=cfg.bars, beats_per_bar=cfg.beats_per_bar)

    preview = work_dir / "DropForge_Preview_Drums_Bass.wav"
    make_preview(drums_clip, bass_clip, preview)

    analysis = Analysis(
        bpm=round(section.bpm, 3), key=key.name, mode=key.mode,
        section_start=round(section.start, 3), section_end=round(section.end, 3),
        source_title=title, source_url=source_url,
        confidence={"key_profile_correlation": round(key.score, 4), "timing_method": section.method},
    )
    ...
    write_json(manifest, {
        "app": "DropForge", "version": APP_VERSION,
        "analysis": analysis.to_dict(),
        "notes": {"drums": len(drums), "bass": len(bass), "bass_raw": len(bass_raw)},
    ...
    return analysis, OutputBundle(
        work_dir, drums_midi, bass_midi, bass_raw_midi,
        drums_clip, bass_clip, preview, manifest, diagnostics_json,
    )
