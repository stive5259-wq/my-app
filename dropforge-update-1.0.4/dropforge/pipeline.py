...
from .bass_evidence import write_evidence_svg
...
def run_pipeline(work_dir: Path, *, url: str | None = None, local_path: Path | None = None, progress: Progress=lambda *_: None) -> tuple[Analysis, OutputBundle]:
    ...
    # Key remains useful producer metadata, but 1.0.4 no longer uses it to rewrite
    # bass pitches.
    ...
    progress("bass", 0.68, "Building Basic Pitch + F0 bass evidence")
    bass, bass_raw, bass_diag, bass_evidence, pyin_track, contour_track = transcribe_bass(
        bass_clip,
        section.bpm,
        0.0,
        duration,
        key,
        kick_rel_raw,
        cfg.bass_onset_threshold,
        cfg.bass_frame_threshold,
        cfg.bass_min_note_ms,
        cfg.sidechain_bridge_ms,
        0.0,
        cfg.bass_decode_min_midi,
        cfg.bass_decode_max_midi,
        f0_min_hz=cfg.f0_min_hz,
        f0_max_hz=cfg.f0_max_hz,
        f0_target_sr=cfg.f0_target_sr,
        f0_frame_length=cfg.f0_frame_length,
        f0_hop_length=cfg.f0_hop_length,
        f0_pitch_tolerance_semitones=cfg.f0_pitch_tolerance_semitones,
        opportunity_onset_tolerance_ms=cfg.opportunity_onset_tolerance_ms,
        opportunity_min_overlap=cfg.opportunity_min_overlap,
        octave_decision_margin=cfg.octave_decision_margin,
    )
    ...
    bass_evidence_json = work_dir / "bass_evidence.json"
    write_json(bass_evidence_json, bass_evidence)
    bass_evidence_svg = work_dir / "bass_evidence.svg"
    write_evidence_svg(
        bass_evidence_svg,
        duration=duration,
        raw_notes=bass_raw,
        fused_notes=bass,
        pyin=pyin_track,
        contour=contour_track,
        kicks=kick_rel_raw,
        min_midi=cfg.bass_decode_min_midi,
        max_midi=cfg.bass_decode_max_midi,
    )
    ...
    progress("midi", 0.88, "Writing register-preserving MIDI")
    ...
    write_json(manifest, {
        ...
        "bass_evidence": {
            "json": bass_evidence_json.name,
            "svg": bass_evidence_svg.name,
            "register_policy": "preserve_absolute_midi",
            "pitch_changes": bass_evidence["summary"]["pitch_changes"],
            "ambiguous_kept": bass_evidence["summary"]["ambiguous_kept"],
        },
        "stems": {
        ...
        "caveats": [
            ...
            "Bass pitch decisions combine Basic Pitch events, Basic Pitch contour, pYIN and harmonic evidence; ambiguous cases keep the strongest Basic Pitch candidate rather than forcing a correction.",
            "Absolute bass register is preserved; DropForge 1.0.4 no longer forces notes into C1-C3.",
            "Estimated key is display metadata in this pass and does not automatically snap chromatic bass notes.",
