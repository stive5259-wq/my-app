...
def _allowed_job_file(job_id: str, filename: str) -> Path:
    job = manager.jobs.get(job_id)
    if not job or not job.result:
        raise HTTPException(404, "Job/file not ready")
...
def get_file(job_id: str, filename: str):
    path = _allowed_job_file(job_id, filename)
    return FileResponse(path, filename=filename)

@app.get("/api/jobs/{job_id}/inline/{filename}")
def inline_file(job_id: str, filename: str):
    path = _allowed_job_file(job_id, filename)
